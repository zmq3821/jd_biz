...
class ProductRequest extends Request
{
    ...
    public function searchProduct($params = [])
    {
        ...
        foreach ($paramsRequire as $key=>$val) {
            ...
            if(is_array($val) && !in_array($params[$key], $val)){
                $tips = '['.implode(',', $val).']';
    ...
    /**
     * 商品可采校验接口
     * 客户商品详情页面、加入购物车以及下单的时候，
     * 校验商品是不是可采购：包括是否在商品池、是否主站上架状态、是否预约预售、是否合同支持购买此商品、是否区域限售。
     * 所以整合商品可售验证+商品上下架接口+商品区域销售限制的校验，上述校验全都通过才会告诉客户商品可采购。
     * 本接口仅用时实时校验，不建议客户将可采状态存库
     * https://bizapi.jd.com/api/product/totalCheckNew
     * @param $skuIds
     * @param $province
     * @param $city
     * @param $county
     ...
     */
    public function totalCheckNew(array $skuIds, $province = 0, $city = 0, $county = 0, $town = 0)
    {
        if(count($skuIds) > 100){
            return HelperLib::returnMsg(1, '最多支持100种商品');
        }
        $params = [
            'skuIds' => implode(',', $skuIds),
            'province' => $province,
        ...
        return $this->queryApi('api/product/totalCheckNew', $params);
    }
