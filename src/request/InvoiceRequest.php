...
class InvoiceRequest extends Request
{
    ...
    public function createInvoice(array $invoice)
    {
        ...
        return $this->queryApi('api/invoice/submit', $invoice);
    }
    ...
    {
        $result = $this->queryApi('api/invoice/select', ['markId' => $markId]);
        $result['data'] = isset($result['data']) ? (is_array($result['data']) ? $result['data'] : json_decode($result['data'], true)) : [];
        return $result;
    }
    ...
    /**
     * 按发票号查询发票物流信息。
     * https://bizapi.jd.com/api/invoice/queryLogisticsByInvoiceNumber
     * @param $invoiceNumber 发票号码
     * @param $invoiceCode 发票代码
     * @return array
     */
    public function queryLogisticsByInvoiceNumber($invoiceNumber, $invoiceCode)
    {
        $params = [
            'invoiceNumber' => $invoiceNumber,
            'invoiceCode'   => $invoiceCode
        ];
        return $this->queryApi('api/invoice/queryLogisticsByInvoiceNumber', $params);
    }
