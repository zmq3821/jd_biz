...
class QueryTools
{
    ...
    public static function query($url, $params = [], $headers = ['Content-Type' => 'application/x-www-form-urlencoded'])
    {
        ...
        if($responseData['success'] !== true){
            ...
            return HelperLib::returnMsg($responseData['resultCode'] ?? 4005, $responseData['resultMessage'] ?? '');
        }
        return HelperLib::returnSuc($responseData['result'] ?? [], $responseData['resultMessage'] ?? '');
    }
